...
const popups: Record<PopupKey, PopupContent> = {
  awareness: {
    title: "🔍 검색 노출",
    color: "var(--gW)",
    sub: '잠재 환자가 우리 병원을 발견하는 모든 접점.<br><strong style="color:#FF8A50">네이버 플레이스가 신규 유입의 60~80%를 차지합니다.</strong>',
    items: [
      {
  ...
  return (
    <>
      ...
      <div className="hero">
        ...
        <h1>
          노크 병원 마케팅 <span className="ac">시스템</span>
        </h1>
        <p>
      ...
      <div className="container">
        <div className="main-row">
          <div className="pipeline-col">
            <div className="sec-label">Patient Acquisition</div>
            <div className="sec-title">환자가 찾아오는 구조를 만듭니다</div>
            <div className="sec-desc">인지 → 관심 → 내원까지, 자동화된 유입 파이프라인</div>

            <div className="pipe-flow">
              {/* 1. 검색 노출 — 강조 */}
              <div className="pipe-row">
                <button className="pipe-node pipe-star" style={{ ["--nc" as string]: "var(--gW)" }} onClick={() => setActivePopup("awareness")} type="button">
                  <div className="pipe-dot">🔍</div>
                  <div className="pipe-info">
                    <div className="pipe-name">검색 노출</div>
                    <div className="pipe-sub">플레이스 · 블로그 · SNS</div>
                  </div>
                  <span className="pipe-badge-star">핵심 ★</span>
                </button>
                <div className="pipe-link"><span className="pipe-link-dot" style={{ background: "var(--gW)" }} /></div>
                <div className="pipe-insight" style={{ ["--ic" as string]: "var(--gW)" }}>
                  <span>💡 네이버 지도 MAU <strong>2,650만</strong> — 환자 68%가 여기서 병원을 검색합니다</span>
                </div>
              </div>

              <div className="pipe-line"><span/></div>

              {/* 2. 문의 유입 */}
              <div className="pipe-row">
                <button className="pipe-node" style={{ ["--nc" as string]: "var(--gP)" }} onClick={() => setActivePopup("lead")} type="button">
                  <div className="pipe-dot">📋</div>
                  <div className="pipe-info">
                    <div className="pipe-name">문의 유입</div>
                    <div className="pipe-sub">전화 · 카톡 · 예약</div>
                  </div>
                </button>
                <div className="pipe-link"><span className="pipe-link-dot" style={{ background: "var(--gP)" }} /></div>
                <div className="pipe-insight" style={{ ["--ic" as string]: "var(--gP)" }}>
                  <span>💡 스마트콜 + 카톡 채널 설정으로 유입 경로를 <strong>100% 추적</strong>합니다</span>
                </div>
              </div>
              ...
              <div className="pipe-row">
                <div className="pipe-pair">
                  <button className="pipe-node" style={{ ["--nc" as string]: "var(--gPu)" }} onClick={() => setActivePopup("nurture")} type="button">
                    <div className="pipe-dot">💬</div>
                    <div className="pipe-info">
                      <div className="pipe-name">환자 설득</div>
                      <div className="pipe-sub">안내 · 재연락</div>
                    </div>
                  </button>
                  <span className="pipe-plus">+</span>
                  <button className="pipe-node" style={{ ["--nc" as string]: "var(--gG)" }} onClick={() => setActivePopup("qualified")} type="button">
                    <div className="pipe-dot">📅</div>
                    <div className="pipe-info">
                      <div className="pipe-name">방문 예정</div>
                      <div className="pipe-sub">예약 확정 · 내원 준비</div>
                    </div>
                  </button>
                </div>
                <div className="pipe-link"><span className="pipe-link-dot" style={{ background: "var(--gPu)" }} /></div>
                <div className="pipe-insight" style={{ ["--ic" as string]: "var(--gPu)" }}>
                  <span>💡 부재중 콜백 ARS 하나로 이탈 환자 <strong>30%를 되돌립니다</strong></span>
                </div>
              </div>
              ...
              {/* 4. 신환 확보 */}
              <div className="pipe-row">
                <button className="pipe-node pipe-convert" style={{ ["--nc" as string]: "var(--gG)" }} onClick={() => setActivePopup("convert")} type="button">
                  <div className="pipe-dot">🏥</div>
                  <div className="pipe-info">
                    <div className="pipe-name">신환 확보</div>
                    <div className="pipe-pills">
                      <span className="pipe-pill">🪑 내원</span>
                      <span className="pipe-pill">📋 상담</span>
                      <span className="pipe-pill">✅ 치료결정</span>
                    </div>
                  </div>
                </button>
                <div className="pipe-link"><span className="pipe-link-dot" style={{ background: "var(--gG)" }} /></div>
                <div className="pipe-insight" style={{ ["--ic" as string]: "var(--gG)" }}>
                  <span>💡 실제 사례: 마케팅 도입 후 <strong>매출 300% 향상</strong></span>
                </div>
              </div>
            </div>
            ...
            <div className="pipe-crm-banner">
              <span>♻️ 이탈 환자 재연락 가능</span>
              <span className="pipe-crm-sep">·</span>
              <span>📱 CRM으로 관리</span>
          ...
          <div className="knock-system-col">
            <div className="sec-label">Customer Success 360</div>
            <div className="sec-title">한 번 온 환자, 계속 오게 만듭니다</div>
            <div className="sec-desc">재방문 · 추천 · 리뷰, 순환하는 성장 엔진</div>
        ...
        <div className="bottom-cta">
          ...
          <div className="cyc-row">
            ...
            <span className="cyc-a">→</span>
            <span className="cyc-pill">설득+방문</span>
            <span className="cyc-a">→</span>
